...
test.describe('AI Country Knowledge System - Balanced Test Suite', () => {
  test.beforeEach(async ({ page }) => {
    // Set reasonable timeout for AI responses
  ...
  test('UI interaction test - cards functionality', async ({ page }) => {
    ...
    // Test clicking on a suggestion (no API call needed)
    const suggestion = page.locator('text=What is the capital of India?');
    await suggestion.click();
    await expect(page.locator('input').first()).toHaveValue('What is the capital of India?');
  });
